...
internal readonly record struct ControllerInputFrame(ushort ControllerIndex, ControllerState State);

/// <summary>Controller feedback sent back to one client-owned controller stream.</summary>
internal readonly record struct ControllerFeedbackFrame(ushort ControllerIndex, ControllerFeedback Feedback);

/// <summary>A controller hot-path pipe message.</summary>
internal readonly record struct ControllerPipeMessage(
...
internal enum ControllerPipeFrameType
{
...
internal sealed class ControllerPipeWriter(Stream stream)
{
...
internal sealed class ControllerPipeReader(Stream stream)
{
    private readonly byte[] _buffer = new byte[ControllerPipeFrame.Size];
