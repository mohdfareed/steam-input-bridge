...
namespace SteamInputBridge {
    ...
    bool MouseReport::hasInput() const { return buttons != 0 || deltaX != 0 || deltaY != 0 || wheel != 0; }
    ...
    bool BridgeProtocolReader::read(uint8_t value, BridgeMessage& message) {
        ...
        if (_offset == 5 && _frame[4] != HandshakeProbeType && _frame[4] != MouseReportType) {
            reset(value);
            return false;
        }
    ...
    bool BridgeProtocolReader::validate() const {
        ...
        const uint16_t actual = readUInt16LittleEndian(_frame + HeaderSize + payloadSize);
    ...
    uint8_t BridgeProtocolReader::expectedFrameSize() const {
        return _offset < HeaderSize ? HeaderSize : static_cast<uint8_t>(HeaderSize + _frame[6] + ChecksumSize);
    }
    ...
    BridgeMessage BridgeProtocolReader::readMessage() const {
        ...
        return BridgeMessage{BridgeMessageType::MouseReport, _frame[5], readMouseReport()};
    }
    ...
    uint8_t writeHandshakeResponse(uint8_t sequence, uint8_t* destination, uint8_t length) {
        ...
        for (uint8_t i = 0; i < BridgeProtocolReader::HandshakeResponsePayloadSize; i++) {
            destination[BridgeProtocolReader::HeaderSize + i] = HandshakeResponsePayload[i];
        }

        const uint16_t checksum =
            crc16(destination, BridgeProtocolReader::HeaderSize + BridgeProtocolReader::HandshakeResponsePayloadSize);
        writeUInt16LittleEndian(
            destination + BridgeProtocolReader::HeaderSize + BridgeProtocolReader::HandshakeResponsePayloadSize,
            checksum);
