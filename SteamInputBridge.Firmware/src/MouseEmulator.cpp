...
namespace SteamInputBridge {
    ...
    void MouseEmulator::emitButton(uint16_t nextButtons, uint16_t previousButtons, uint16_t bridgeButton,
                                   uint8_t teensyButton) {
        if ((nextButtons & bridgeButton) == (previousButtons & bridgeButton)) {
            return;
        }

        (nextButtons & bridgeButton) != 0 ? Mouse.press(teensyButton) : Mouse.release(teensyButton);
    }
