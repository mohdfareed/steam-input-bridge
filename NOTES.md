...
| Steam can echo VIIPER DS4 outputs as new generic Steam `PS4 Controller` entries. | If the initial client baseline had no generic Steam DS4, later generic Steam DS4 entries are loopback and must not be opened. | `Hosting/Client/Run/Controllers/ClientControllerSourceRegistrar.cs`, `Hosting/SdlControllerRoutePolicy.cs` | `SdlControllerRoutePolicyTests` |
...
| Steam can echo VIIPER DS4 outputs as generic PS4 controller streams. | After host physical resolution, unresolved Steam-routed `054c:05c4` `PS4 Controller` streams are recursive loopback and must not register routes. | `Hosting/Server/Pipes/ClientControllerPipe.Controllers.cs` | `HostingForwardingTests` |
...
| HidHide normal mode uses one global app allowlist for every hidden device. | Register this executable once, keep it on the app list, hide scoped physical controllers, temporarily remove other app-list entries, force inverse mode off, then restore previous state. | `HidHide/HidHideService.cs` | `HidHideServiceTests` |
...
| HidHide app-list state is user-owned global state. | Do not remove app entries at startup. Only mutate them inside an active scope snapshot that can restore the previous list. | `HidHide/HidHideService.cs` | `HidHideServiceTests` |
