...
| HidHide normal mode uses one global app allowlist for every hidden device. | Register this executable, HidHideCLI.exe, `steam.exe`, and `SteamService.exe` once, keep them on the app list, hide scoped physical controllers, force inverse mode off, then restore only device and mode state. Do not prune the user's app list. | `HidHide/HidHideService.cs`, `Hosting/Server/ServerServices.cs` | `HidHideServiceTests` |
...
| Steam Input can need Steam's own controller processes on HidHide's app list. | Allowlist `steam.exe` and `SteamService.exe` with the app. Keep `steamwebhelper.exe` off unless testing proves the CEF UI process owns controller visibility. | `Hosting/Server/ServerServices.cs` | Manual/log validated |
...
| HidHide app-list state is user-owned global state. | Do not remove app entries at startup or during profile scopes. The only normal mutation is adding required app/Steam executables if any are missing. | `HidHide/HidHideService.cs` | `HidHideServiceTests` |
